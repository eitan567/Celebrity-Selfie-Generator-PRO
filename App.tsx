    ...
    if (!selectedTemplate) {
        return (
            <div className="min-h-screen bg-gray-50 font-heebo">
                ...
                <footer className="text-center text-gray-400 text-sm py-8">Powered by Google Gemini 2.5</footer>
            </div>
        );
    }
            ...
            <header className={`bg-gradient-to-l ${selectedTemplate.colorFrom} ${selectedTemplate.colorTo} text-white p-6 shadow-lg z-20 shrink-0`}>
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button onClick={exitToSelection} className="bg-white/20 hover:bg-white/30 p-2 rounded-lg transition backdrop-blur cursor-pointer z-50" title="חזרה לתפריט ראשי"><Icon name="Home" size={24} /></button>
                        <div>
                    ...
                    <div className="bg-black/20 px-4 py-2 rounded-lg backdrop-blur">
                        ...
                        <span className={`font-bold ${isProcessing ? (isPausedForApproval ? 'text-blue-200' : 'text-green-300 animate-pulse') : 'text-gray-200'}`}>{isProcessing ? (isPausedForApproval ? 'ממתין לאישור' : 'מעבד...') : 'מוכן'}</span>
                    </div>
                </div>
            </header>
